...
import static mta.computional.slanguage.simpl.instruction.function.factory.SFunction.NOT;
import static mta.computional.slanguage.smodel.api.label.ConstantLabel.EXIT;

public class RemainingFunction extends SProgramImpl {

    public RemainingFunction() {
        super(REMAINDER.toString());
        ...
        addInstruction(SComponentFactory.createInstruction(JUMP_NOT_ZERO, z2, AdditionalArguments.builder().jumpNotZeroLabel(B).build()));
        ...
        addInstruction(createInstructionWithLabel(B, ASSIGNMENT, "y", AdditionalArguments.builder().assignedVariableName(remaining).build()));
